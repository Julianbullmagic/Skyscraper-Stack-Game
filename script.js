...
function generateBox(x, y, z, width, depth, falls) {
  ...
  var geometry = new THREE.BoxGeometry(width, boxHeight, depth);
  		...
  		// creating texture materials
    	const materials = [
  				new THREE.MeshBasicMaterial({map: loader.load('skyscraper.jpg')}),
  				new THREE.MeshBasicMaterial({map: loader.load('skyscraper.jpg')}),
  				new THREE.MeshBasicMaterial({map: loader.load('boxtop.jpg')}),
  				new THREE.MeshBasicMaterial({map: loader.load('skyscraper.jpg')}),
  				new THREE.MeshBasicMaterial({map: loader.load('skyscraper.jpg')}),
  				new THREE.MeshBasicMaterial({map: loader.load('skyscraper.jpg')}),
  		];
...
console.log(stack.length-6)
    if (stack.length>7){scoreElement.innerText = stack.length - 6}
    else{scoreElement.innerText = 0}
...
function animation(time) {
  if (lastTime) {
    ...
    const speed = 0.008;
    backgroundPos-=0.0005
    document.documentElement.style.setProperty('--background-scroll', backgroundPos + "%");
